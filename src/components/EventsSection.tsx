...
const EventsSection = () => {
  ...
  const eventIcons = [FaCamera, FaVideo, FaUserFriends]
  const benefitIcons = [FaCheckCircle, FaGift, FaTrophy]
  const trustBadgeIcons = [FaStar, FaUsers, FaGlobeAsia, FaPercent]
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="text-xl font-light text-slate-300 md:text-2xl">
            <span className="font-semibold text-white">{t.events.subtitle}</span>{t.events.subtitleEnd}
          </p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-8 shadow-2xl md:p-12 relative overflow-hidden">
          ...
          <div className="text-center relative z-10">
            ...
            <p className="mb-8 text-lg text-slate-300">
              {t.events.ctaSubtitle} <span className="font-bold text-ocean-teal">{t.events.ctaHighlight}</span>
            </p>
            ...
            <div className="flex flex-col items-center justify-center gap-4 sm:flex-row mb-8">
              ...
              >
                {t.events.instagramButton}
              </a>
            </div>

            {/* Additional Benefits */}
            <div className="grid gap-4 md:grid-cols-3">
              {t.events.benefits.map((benefit, index) => {
                const IconComponent = benefitIcons[index]
                const iconColors = ['text-ocean-teal', 'text-parks-gold', 'text-yellow-500']
                return (
                  <div key={index} className="rounded-xl bg-white/5 border border-white/10 p-4 hover:bg-white/10 transition-colors">
                    <IconComponent className={`mb-2 text-2xl ${iconColors[index]} mx-auto`} />
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex flex-wrap items-center justify-center gap-6 rounded-2xl bg-white/5 border border-white/10 p-6 backdrop-blur-sm">
            {t.events.trustBadges.map((badge, index) => {
              const IconComponent = trustBadgeIcons[index]
              const iconColors = ['text-parks-gold', 'text-white', 'text-white', 'text-white']
              return (
                <React.Fragment key={index}>
                  <div className="flex items-center gap-2">
                    <IconComponent className={`text-2xl ${iconColors[index]}`} />
                    <span className="font-bold text-white">{badge}</span>
                  </div>
                  {index < t.events.trustBadges.length - 1 && (
